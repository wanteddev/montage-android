---
title: icon-button
description: 아이콘에 배경 원형 효과와 터치 영역을 추가한 아이콘 버튼 컴포저블입니다.
image: /components/icon-button/design/thumbnail.png
createdAt: 2023-11-01
---

## WantedIconButtonBackground
아이콘에 배경 원형 효과와 터치 영역을 추가한 아이콘 버튼 컴포저블입니다. 활성/비활성 상태, 대체 배경 여부, 아이콘 색상 등 시각적 스타일을 지정할 수 있으며, 터치 영역이 확대되어 사용자 편의성을 높입니다.
...
원형 배경 없이 단순한 아이콘만을 표시하는 기본 아이콘 버튼입니다. 필요 시 우측 상단에 PushBadge 컴포저블을 함께 표시할 수 있으며, 터치 영역은 `WantedTouchArea`를 통해 넉넉하게 설정되어 있습니다.
...
## WantedIconButtonOutlined (패딩 수동 지정)
패딩을 수동 지정할 수 있는 Outlined 아이콘 버튼입니다. 이 함수는 직접 `size` 대신 Modifier 크기 및 padding을 지정하여 좀 더 유연한 스타일링이 가능합니다.
...
`WantedIconButtonSize`를 사용하여 간편하게 크기와 패딩을 지정할 수 있는 Solid 스타일 아이콘 버튼입니다. 원형 배경과 흰색 아이콘을 기본으로 하며, 배경/아이콘 색상은 커스터마이징이 가능합니다.
...
| modifier | `Modifier` | 외형 및 배치를 제어하는 Modifier입니다. |
| enabled | `Boolean` | 버튼 활성화 여부입니다. |
| tint | `Color` | 아이콘 색상입니다. 기본값은 흰색입니다. |
| background | `Color` | 배경 색상입니다. 기본값은 primary_normal입니다. |
| onClick | `() -> Unit` | 클릭 시 호출되는 콜백입니다. |

---

## WantedIconButtonSolid (패딩 수동 지정)
...
## Note
아이콘 버튼의 크기 및 내부 패딩을 정의하는 enum 클래스입니다. Medium - 표준 사이즈 (40dp)로, 대부분의 UI에 적합합니다. Small - 소형 사이즈 (32dp)로, 공간이 제한된 영역에 적합합니다. 각 값은 버튼의 전체 크기(`size`)와 아이콘과 배경 간의 간격을 나타내는 `padding`을 제공합니다.
